    ...
    contract Goals {
        ...
        mapping(uint256 => mapping(address => uint256)) public participantsAutenticatedFreq; // Link goal id to the number of times a participant had their frequency authenticated
        ...
        function enterGoal(uint256 _goalId, uint256 _bet) public {
            ...
            require(
                token.balanceOf(msg.sender) >= _bet,
                "Insufficient funds"
            );
            require(
        ...
        function updateFrequency(uint256 _goalId, string memory _imgaeURI) public {
            ...
            require(participantsFreq[_goalId][msg.sender] < goals[_goalId].targetFreq, "You have reached the maximum number of frequencies");

            participantsURI[_goalId][msg.sender].push(_imgaeURI);
            participantsFreq[_goalId][msg.sender] += 1;
        }

        function autenticateFrequency(uint256 _goalId, address _participant, uint _number, uint []  memory _uriIndex) public {
            require(
            ...
            require(goals[_goalId].isStarted == true, "Goal has not started yet");
            // require(goals[_goalId].endDate > block.timestamp, "Goal has ended");
            require(participantsAutenticatedFreq[_goalId][_participant] < goals[_goalId].targetFreq, "This participant has reached the maximum number of frequencies");
        ...
        function completeGoal(uint256 _goalId) public {
            ...
            require(goals[_goalId].isCompleted == false, "Goal has already been completed");
            ...
            for (uint i = 0; i < totalParticipants; i++) {
                uint partFreq = participantsAutenticatedFreq[_goalId][participants[_goalId][i]];
            ...
            if (numPremParts == 0) {
                ...
                for (uint i = 0; i < totalParticipants; i++) {
                    uint partFreq = participantsFreq[_goalId][participants[_goalId][i]];
                    if (partFreq == goalFreq) {
        ...
        function getParticipants(uint256 _goalId) public view returns(address[] memory) {
            return participants[_goalId];
        }

        function getParticipantsUri(uint256 _goalId, address _participant) public view returns(string[] memory) {
            return participantsURI[_goalId][_participant];
        }

        function getAutenticatedUri(uint256 _goalId, address _participant) public view returns(uint[] memory) {
